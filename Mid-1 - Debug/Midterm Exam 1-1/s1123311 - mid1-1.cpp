...
vector::iterator vector::insert( const_iterator where, const value_type &val )
{
   if( where >= myFirst && where <= myLast )
   {
      ...
      if (originalSize == originalCapacity)
      {
          ...
          size_type position = where - myFirst;    // modify
      ...
      {
          size_type position = where - myFirst;
          value_type buffer,tmp=myFirst[position];//add

          myFirst[position] = val;

          for (size_t i = position+ 1; i <originalSize + 1; i++)//modify
          {
