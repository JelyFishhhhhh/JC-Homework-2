...
vector::iterator vector::insert( const_iterator where, const value_type &val )
{
   if( where >= myFirst && where <= myLast )
   {
      ...
      if( originalSize == originalCapacity )
      {
         ...
         value_type* buffer = new value_type[newCapacity];
         size_t num = where - myFirst;
         for (size_t i = 0; i < num; i++)
         {
             buffer[i] = myFirst[i];
         }
         buffer[num] = val;
         for (size_t i = num; i < originalSize; i++)
         {
             buffer[i+1] = myFirst[i];
         }
         delete[] myFirst;
         myFirst = buffer;
         ...
         return myFirst + num;//add
      }
      else
      {
          /*value_type* buffer = new value_type[originalCapacity];
          size_t num = where - myFirst;
          for (size_t i = 0; i < num; i++)
          {
              buffer[i] = myFirst[i];
          }
          buffer[num] = val;//modify
          for (size_t i = num; i < originalSize; i++)
          {
              buffer[i + 1] = myFirst[i];
          }
          delete[] myFirst;
          myFirst = buffer;*/
          value_type buffer, temp;
          temp = val;
          for (size_t i = where - myFirst; i < originalSize + 1; i++)
          {
              buffer = myFirst[i];
              myFirst[i] = temp;
              temp = buffer;
          ...
          return myFirst + (where - myFirst);//add
      }
   }
   else
      return nullptr;
