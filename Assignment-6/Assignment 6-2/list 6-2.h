   ...
   list& operator=( const list &right )
   {
      if( this != &right )
      {
         ...
         else // the right list is not empty
         {
            ...
            nodePtr leftNode= myData.myHead->next;
            nodePtr rightNode= right.myData.myHead->next;
            nodePtr leftPrevNode= myData.myHead;

            while (rightNode!= right.myData.myHead && leftNode!= myData.myHead){
               
               leftNode->myVal= rightNode->myVal;
               leftPrevNode= leftNode;
               leftNode= leftNode->next;
               rightNode= rightNode->next;
            }
            
            while(rightNode!= right.myData.myHead){

               nodePtr newNode= new node;
               
               // newNNode assignment
               newNode->myVal= rightNode->myVal;
               newNode->next= myData.myHead;
               newNode->prev= myData.myHead->prev;

               leftPrevNode->next= newNode;
               myData.myHead->prev= newNode;
               leftPrevNode= newNode;

               rightNode= rightNode->next;
            }

            while(leftNode!= myData.myHead){

               leftPrevNode->next= myData.myHead;
               myData.myHead->prev= leftPrevNode;

               nodePtr temp= leftNode;
               leftNode= leftNode->next;
