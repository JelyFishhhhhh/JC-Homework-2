...
{
    int sumY = 0;
    for (int i = this->year; i < date2.year; i++) {

        sumY += leapYear(i) ? 366 : 365;
    }
    int sum1 = this->day+ (this->year== date2.year? 0: (leapYear(date2.year)? 366: 365));
...
Date Date::operator+( int numDays )
{
    Date newDay = *this;
    newDay.day += numDays;
    if (days[this->month] < newDay.day) {

        newDay.month++;
        newDay.day -= days[newDay.month]- 1;
    }
    if (newDay.month > 12) {
